...
// Grab start address and end address from the URL
var startAddress = url.searchParams.get("start");
var endAddress = url.searchParams.get("end");

// If startAddress and endAddress are not defined, default
...
map.on('load', function () {
    if (startAddress == null) {
        startAddress = default_startAddress;
    }
    if (endAddress == null) {
        endAddress = default_endAddress;
    }

    // Starts a series of asynchronous API calls
    // get_startCoordinates is first,
    // get_endCoordinates is second,
    // startDrawingPoints is last but calls getRoute twice which contain more API calls
...
function get_startCoordinates() {
    ...
    var parameter = default_startAddress.split(',').join("");
...
function get_endCoordinates() {
    address = endAddress;
    var parameter = default_endAddress.split(',').join("");
    parameter = encodeURIComponent(parameter.trim())
    var url = "https://api.mapbox.com/geocoding/v5/mapbox.places/" + address + ".json?country=US&access_token=" + mapboxgl.accessToken;
    var req = new XMLHttpRequest();
    req.open('GET', url);
...
// create a function to make a directions request and draw the route
...
// Create icons at the start and end point
function drawStartingPoints() {
    // Draw the route between the two points, needs to be called twice
    ...
    el_start.style.backgroundImage = 'url(images/home.png)';
    ...
    el_end.style.backgroundImage = 'url(images/map-pin.png)';
...
/*
// Click to create destination location
